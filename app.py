import os, shutil, tempfile, zipfile, uuid, threading, time, json   ### nuevo json
...
# ─── Job store ───────────────────────────────────────────
JOBS: dict[str, dict] = {}   # job_id → dict estado


# ─── Helpers (sin cambios) ───────────────────────────────
def allowed(fn:str)->bool: return Path(fn).suffix.lower() in ALLOWED
def unzip(fs,dest:Path)->List[Path]:
    ztmp=dest/secure_filename(fs.filename); fs.save(ztmp); outs=[]
    with zipfile.ZipFile(ztmp) as zf:
        for m in zf.namelist():
            if Path(m).suffix.lower() in (".pdf",".jpg",".jpeg",".png"):
                ...
                outs.append(out)
    ztmp.unlink(); return outs
def folder_for(info:Dict[str,str])->Path:
    p=OUT_DIR/info.get("NIVEL","OTROS").upper().replace(" ","_"); p.mkdir(exist_ok=True); return p
def write_zip(rows):  # idem
    with zipfile.ZipFile(OUT_DIR/"certificados_clasificados.zip","w") as zf:
        for r in rows:
            if r["ESTADO"]=="OK": zf.write(OUT_DIR/r["ARCHIVO"],arcname=r["ARCHIVO"])
def write_excel(rows): pd.DataFrame(rows).to_excel(OUT_DIR/"certificados.xlsx", index=False)

# ─── Worker con mensajes de progreso ─────────────────────
def process_job(jid:str, paths:List[Path], modo:str):
    job=JOBS[jid]; total=len(paths)
    job.update(pct=5, msg="Leyendo archivos…")                      ###

    rows=[]; step=60/total if total else 0
    for idx,p in enumerate(paths,1):
        ...
        if info["ESTADO"]=="OK":
            ...
            if p.suffix.lower() in (".jpg",".jpeg",".png"):
                with open(target,"wb") as f: f.write(img2pdf.convert(str(p)))
    ...
    write_zip(rows); write_excel(rows)
...
@app.post("/start")
def start():
    modo=request.form.get("tipo_cert","auto"); ups=request.files.getlist("files")
    if not ups: flash("Sin archivos","warning"); return redirect(url_for("index"))
    if len(ups)>MAX_UP: flash("Demasiados archivos","danger"); return redirect(url_for("index"))
    tmp=Path(tempfile.mkdtemp()); paths=[]
    for fs in ups:
        if not allowed(fs.filename): flash(f"No permitido: {fs.filename}","danger"); continue
        paths += unzip(fs,tmp) if fs.filename.lower().endswith(".zip") else [tmp/secure_filename(fs.filename)]
        if not fs.filename.lower().endswith(".zip"): paths[-1].write_bytes(fs.read())
...
def progreso(job_id):
    if job_id not in JOBS: flash("Job inexistente","danger"); return redirect(url_for("index"))
...
def events(job_id):
    def stream():
        while True:
            job=JOBS.get(job_id);  time.sleep(0.3)
            if not job: break
...
def resultado(job_id):
    job=JOBS.get(job_id)
    if not job or not job["finished"]: flash("Aún procesando","warning"); return redirect(url_for("progreso", job_id=job_id))
